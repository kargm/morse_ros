...
for row in posesReader:
    ...
    for obj_row in objectReader:
         if (abs(poses_time - float(obj_row['time'])) < 0.1):
              ...
              obj_string.data = message + ")"
              # publish object detections similar to MORSE Semantic Camera
              obj_pub.publish(obj_string)
              # Also send TF for visualization
              br.sendTransform((float(obj_row['obj_x']), float(obj_row['obj_y']), float(obj_row['obj_z'])),
                               tf.transformations.quaternion_from_euler(0, 0, 0),
                               rospy.Time.now(),
                               str(obj_row['object'][:-2]),
                               "map")
