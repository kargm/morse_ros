...
if [ -z "$PYTHON3_EXECUTABLE" ]; then
    if which python3.3; then
        PYTHON3_EXECUTABLE=`which python3.3`
    else
...
export PYTHONPATH=$PYYAML_DIR/pyyaml/lib/python3.3/site-packages:$MORSE_DIR/morse/$PYTHON_INST_DIR:/usr/$PYTHON_INST_DIR:/opt/ros/fuerte/lib/python2.7/dist-packages
